...
class Endpoint:
    ...
    def _paged(url_template: str):
        def generate_pages(*, count: int, count_per_page: int, offset: int = 0, **kwargs):
            for page_offset in range(0, count, count_per_page):
                yield url_template.format(limit=count_per_page, offset=(offset + page_offset), **kwargs)
    ...
    me = user(user_id="me")
    subscriptions = _paged("https://onlyfans.com/api2/v2/subscriptions/subscribes?limit={limit}&offset={offset}&type=active")
    posts_paid = _paged("https://onlyfans.com/api2/v2/posts/paid?limit={limit}&offset={offset}")
    posts = _paged("https://onlyfans.com/api2/v2/users/{user_id}/posts?limit={limit}&offset={offset}&order=publish_date_desc&skip_users_dups=0")
    posts_archived = _paged("https://onlyfans.com/api2/v2/users/{user_id}/posts/archived?limit={limit}&offset={offset}&order=publish_date_desc")
    messages = _paged("https://onlyfans.com/api2/v2/chats/{user_id}/messages?limit={limit}&offset={offset}&order=desc")
    license = _formatted("https://onlyfans.com/api2/v2/users/media/{media_id}/drm/{response_type}/{post_id}?type=widevine")
...
class SignInfo(pydantic.BaseModel):
    ...
    def from_download(cls) -> SignInfo:
        return SignInfo.model_validate(
            requests.get(Endpoint.sign_info).json()
        )
    ...
    def sign(self, url: str, current_time: str) -> str:
        ...
        checksum = sum(ord(sha1[index]) for index in self.checksum_indexes) + self.checksum_constant
...
class AuthInfo(pydantic.BaseModel):
    ...
    def parse_cookie(cls, value):
        ...
        cookie_dict = dict(item.strip().split('=', 1) for item in value.split(';') if '=' in item)
...
class MPD(pydantic_xml.BaseXmlModel, nsmap=mpd_nsmap):
    ...
    class Period(pydantic_xml.BaseXmlModel, tag="Period", nsmap=mpd_nsmap):
        ...
        class AdaptationSet(pydantic_xml.BaseXmlModel, tag="AdaptationSet", nsmap=mpd_nsmap):
            mimeType: str = pydantic_xml.attr()
            content_protections: list[ContentProtection]
            class ContentProtection(pydantic_xml.BaseXmlModel, tag="ContentProtection", nsmap=mpd_nsmap):
            ...
            class Representation(pydantic_xml.BaseXmlModel, tag="Representation", nsmap=mpd_nsmap):
            ...
            widevine_uri: ClassVar[str] = "urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed"
            def _get_media(self) -> tuple[Representation, ContentProtection]:
                return (
                    ...
                    next(c for c in self.content_protections if c.schemeIdUri == self.widevine_uri)
                )

    def get_media(self, mime_prefix: Literal["video", "audio"]):
        aset = next(a for a in self.period.adaptation_sets if a.mimeType.startswith(mime_prefix))
...
class Session:
    ...
    async def get(self, url: str, cookies: Mapping[str, str] | None = None):
        return await self.session.get(url, headers=self.make_headers(url), cookies=cookies)
    ...
    async def head(self, url: str, cookies: Mapping[str, str] | None = None):
        return await self.session.head(url, headers=self.make_headers(url), cookies=cookies)
...
class OnlyFans:
    ...
    def __init__(self, auth: AuthInfo | dict, cdm: CDMInfo | dict | None = None, sign: SignInfo | dict | None = None):
    ...
    async def get_user(self, user_id: int) -> User:
        ...
        user = User.model_validate(await self.session.get_json(Endpoint.user(user_id=user_id)))
    ...
    async def get_subscriptions(self) -> list[Subscription]:
        ...
        coroutines = [
            ...
            for url in Endpoint.subscriptions(count=self.me.subscribesCount, count_per_page=self.count_per_page)
        ]
        subscriptions = [Subscription.model_validate(sub) for subs in await asyncio.gather(*coroutines) for sub in subs]
        assert(len(subscriptions) == self.me.subscribesCount), f"Expect {self.me.subscribesCount} subscriptions, got {len(subscriptions)}"

        self.subscriptions = subscriptions
        return subscriptions

    async def get_posts(self, user_id: int, archived: bool = False, progress: tqdm | None = None) -> list[Post]:
        ...
        if (archived and user.archivedPostsCount == 0) or (not archived and user.postsCount == 0):
            return []

        async def request_posts(url: str) -> list[Post]:
            posts = [Post.model_validate(post) for post in await self.session.get_json(url)]
            if progress is not None:
        ...
        coroutines = [
            request_posts(url)
            for url in endpoint(user_id=user_id, count=user.postsCount, count_per_page=self.count_per_page)
        ]
        ...
        assert(len(posts) == expected_count), f"Expect {expected_count} posts, got {len(posts)}"

        cache[user.id] = posts
        return posts

    async def get_messages(self, user_id: int, progress: tqdm | None = None) -> list[Post]:
        ...
        async def request_messages(url: str) -> list[Post]:
            ...
            messages = [Post.model_validate(message) for message in (await self.session.get_json(url))["list"]]
            if progress is not None:
        ...
        for i in range(self.max_batch_count):
            coroutines = [
                ...
                for url in Endpoint.messages(user_id=user_id, count=self.count_per_batch, count_per_page=self.count_per_page, offset=i * self.count_per_batch)
            ]
            messages_list = await asyncio.gather(*coroutines)
            messages.extend(message for messages in messages_list for message in messages)
            # No more messages
    ...
    async def get_content_size(self, url: str, cookies: Mapping[str, str] | None = None) -> int:
        response = await self.session.head(url, cookies=cookies)
        return int(response.headers.get("Content-Length", 0))

    async def get_contents_size(self, urls: list[str], cookie_jars: Iterable[Mapping[str, str]] | None = None) -> AsyncGenerator[int]:
        cookie_jars = cookie_jars or [None] * len(urls)
        for size_future in asyncio.as_completed([self.get_content_size(url, cookies) for url, cookies in zip(urls, cookie_jars)]):
...
class Downloader:
    ...
    async def download_file(self, url: str, progress: tqdm | None = None, cookies: Mapping[str, str] | None = None) -> str:
    ...
    async def download_files(self, urls: Iterable[str], progress: tqdm, cookie_jars: Iterable[Mapping[str, str]] | None = None) -> list[str]:
    ...
    async def download_profile(self):
        ...
        with tqdm(total=0, leave=False, unit="B", unit_scale=True, desc="Downloading avatar and header") as progress:
    ...
    async def download_medias(self, medias: Iterable[Media]) -> tuple[int, int]:
        ...
        all_urls = [media.files.full.url for media in medias if media.files.full.url is not None]
        urls = [url for url in all_urls if not self.is_url_downloaded(url)]
        already_downloaded_count = len(all_urls) - len(urls)

        with tqdm(total=0, leave=False, unit="B", unit_scale=True, desc="Downloading media") as progress:
    ...
    async def download_posts(self) -> tuple[int, int]:
        with tqdm(total=self.user.postsCount + self.user.archivedPostsCount, leave=False, desc="Indexing posts") as progress:
            posts = await self.api.get_posts(self.user.id, archived=False, progress=progress)
            posts += await self.api.get_posts(self.user.id, archived=True, progress=progress)
        return await self.download_medias(media for post in posts for media in post.media)
    ...
    async def download_messages(self):
        ...
        return await self.download_medias(media for message in messages for media in message.media)
    ...
    class DRMMedia:
        ...
        def license_url(self):
            return Endpoint.license(media_id=self.media.id, response_type=self.post.responseType, post_id=self.post.id)

        @property
        def video_url(self):
            return self.media.files.drm.manifest.dash.rsplit("/", 1)[0] + "/" + self.video.base_url

        @property
        def audio_url(self):
            return self.media.files.drm.manifest.dash.rsplit("/", 1)[0] + "/" + self.audio.base_url
    ...
    async def download_medias_drm(self, drm_medias: DRMMedia):
        ...
        progress = tqdm(total=len(drm_medias), leave=False, desc="Indexing drm manifests")
        async def add_manifest(drm: Downloader.DRMMedia):
            response = await self.api.session.get(drm.media.files.drm.manifest.dash, drm.media.files.drm.signature.dash)
            if response.status == 403:
                # CloudFront may return 403 if on VPN (with low reputation IPs)
                raise Exception(f"Permission denied to retrieve manifest. Disconnect any VPN.")
        ...
        progress = tqdm(total=0, leave=False, unit = "B", unit_scale=True, desc="Downloading drm")
        # Filter for drm that are not already downloaded
        drms = [drm for drm in drm_medias if not self.is_file_downloaded(drm.final_filename)]
        ...
        for drm in drms:
            ...
            session_id = cdm.open()
            challenge = cdm.get_license_challenge(session_id, pywidevine.PSSH(drm.protection.pssh))
            ...
            for raw_path, decrypted_path in ((raw_video_path, decrypted_video_path), (raw_audio_path, decrypted_audio_path)):
                ffmpeg.options([
            ...
            progress.update()
            assert os.path.exists(final_path), f"Decryption failed, missing final file {final_path}"
        progress.close()
    ...
    async def download_posts_drm(self) -> tuple[int, int]:
        ...
        drm_medias = [Downloader.DRMMedia(p, m) for p in posts for m in p.media if m.is_drm()]
        if len(drm_medias) == 0:
            return (0, 0)
    ...
    async def download_messages_drm(self) -> tuple[int, int]:
        ...
        drm_medias = [Downloader.DRMMedia(message, media) for message in messages for media in message.media if media.is_drm()]
        if len(drm_medias) == 0:
            return (0, 0)
